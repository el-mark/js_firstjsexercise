...
function findLongestWord( array_words ){
	for (var index = 0, longestlenght = 0 ; index < array_words.length; index++ ){
		if (longestlenght < array_words[index].length) {
			longestlenght = array_words[index].length
	    }
	}
...
array_lengths.push( findLongestWord( ["Computer", "celphone", "mouse", "table"] ) );
array_lengths.push( findLongestWord( ["How are you?", "Are you doing fine?", "How do you slept last night?"] ) );
...
console.log( 6 === 6 && "The average of longest lengths is: " + calculateAverage( array_lengths ))
...
function charFreq( string ) {
	freqCounter = {};
		for (var index = 0, lettercheck = 0 ; index < string.length; index++) {
	  	
    	  	lettercheck = string[index];
    	  	
    	  	for (var index2 = 0, freq = 0; index2 < string.length; index2++) {
    	  	        if (lettercheck === string[index2]) {
    	  	            freq += 1 ;
    	  	        }
    	  	    }
    	  	
    	  	freqCounter[string[index]] = freq;
